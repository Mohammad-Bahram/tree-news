...
import { deleteCategry } from "./actions";
...
const Categories = () => {
  ...
  const mutation = useMutation({
    ...
    onError: () => {
      return <h1>عملیات با مشکل مواجه شد</h1>;
    },
  });
  if (isLoading) {
    return <Loading />;
  ...
  if (isError) {
    return <h1>دوباره امتحان کنید.</h1>;
  }
  ...
  return (
    <div className="w-full h-fit max-lg:h-fit pb-[30px] pt-[24px] bg-white flex flex-col justify-normal items-start rounded-[15px] px-6 shadow-[0px_0px_5px_rgba(0,0,0,11)]">
      ...
      >
        ...
        <div className="flex max-md:flex-col max-md:gap-y-4 max-sm:w-full flex-row gap-x-3 w-full md:w-fit">
          <CategorySelect />
          <CategorySearch />
          <AddCategory modal={true} />
        </div>
      </div>
      {data && (
            ...
            {data?.map((elem: CATEGORY_DATA) => {
              return (
                ...
                >
                  ...
                  <td className="border-none px-4 py-2 max-sm:p-0 text-center rounded-tl-[10px] rounded-bl-[10px]">
