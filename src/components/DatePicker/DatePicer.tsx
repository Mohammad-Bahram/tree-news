import React, { useState } from "react";
import dynamic from "next/dynamic";
import persian from "react-date-object/calendars/persian";
import persian_fa from "react-date-object/locales/persian_fa";
import DateObject from "react-date-object";

// Lazy load the DatePicker component so that it only runs on the client-side
const DatePicker = dynamic(() => import("react-multi-date-picker"), {
  ssr: false,
});

const CustomDatePicker: React.FC<{ title: string }> = ({ title }) => {
  const [date, setDate] = useState<any>(
    new DateObject({ calendar: persian, locale: persian_fa })
  );

  return (
    <div className="flex items-center max-lg:flex-col gap-y-5  ">
      <label className="text-gray-700  w-[90px]  text-md ml-[9px]">
        تاریخ {title}
      </label>

      <div className="relative  h-[50px]  ">
        <DatePicker
          value={date}
          onChange={setDate}
          calendar={persian}
          locale={persian_fa}
          inputClass="w-full h-10 px-4 py-2 border border-gray-300 rounded-[5px] text-right"
          format="YYYY/MM/DD"
          render={(value, openCalendar) => (
            <div
              className="flex  items-center
                  w-[150px]
                  max-lg:w-[240px] lg:w-[215px] justify-between  h-12 px-3 border border-gray-300 rounded-lg cursor-pointer "
              onClick={openCalendar}
            >
              <span className="text-[#282828] text-[18px]">
                {" "}
                {value || "تاریخ انتخاب نشده"}
              </span>
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M8 5.75C7.59 5.75 7.25 5.41 7.25 5V2C7.25 1.59 7.59 1.25 8 1.25C8.41 1.25 8.75 1.59 8.75 2V5C8.75 5.41 8.41 5.75 8 5.75Z"
                  fill="#333333"
                />
                <path
                  d="M16 5.75C15.59 5.75 15.25 5.41 15.25 5V2C15.25 1.59 15.59 1.25 16 1.25C16.41 1.25 16.75 1.59 16.75 2V5C16.75 5.41 16.41 5.75 16 5.75Z"
                  fill="#333333"
                />
                <path
                  d="M8.5 14.5C8.37 14.5 8.24 14.47 8.12 14.42C7.99 14.37 7.89001 14.3 7.79001 14.21C7.61001 14.02 7.5 13.76 7.5 13.5C7.5 13.24 7.61001 12.98 7.79001 12.79C7.89001 12.7 8 12.63 8.12 12.58C8.36 12.48 8.64 12.48 8.88 12.58C9 12.63 9.10999 12.7 9.20999 12.79C9.24999 12.84 9.29999 12.89 9.32999 12.94C9.36999 13 9.40001 13.06 9.42001 13.12C9.45001 13.18 9.47001 13.24 9.48001 13.3C9.49001 13.37 9.5 13.44 9.5 13.5C9.5 13.76 9.38999 14.02 9.20999 14.21C9.10999 14.3 9 14.37 8.88 14.42C8.76 14.47 8.63 14.5 8.5 14.5Z"
                  fill="#333333"
                />
                <path
                  d="M12 14.5C11.87 14.5 11.74 14.47 11.62 14.42C11.49 14.37 11.39 14.3 11.29 14.21C11.11 14.02 11 13.76 11 13.5C11 13.44 11.01 13.37 11.02 13.3C11.03 13.24 11.05 13.18 11.08 13.12C11.1 13.06 11.13 13 11.17 12.94C11.21 12.89 11.25 12.84 11.29 12.79C11.66 12.42 12.33 12.42 12.71 12.79C12.75 12.84 12.79 12.89 12.83 12.94C12.87 13 12.9 13.06 12.92 13.12C12.95 13.18 12.97 13.24 12.98 13.3C12.99 13.37 13 13.44 13 13.5C13 13.76 12.89 14.02 12.71 14.21C12.52 14.39 12.27 14.5 12 14.5Z"
                  fill="#333333"
                />
                <path
                  d="M8.5 18C8.37 18 8.24 17.97 8.12 17.92C8 17.87 7.89001 17.8 7.79001 17.71C7.61001 17.52 7.5 17.26 7.5 17C7.5 16.94 7.50999 16.87 7.51999 16.81C7.52999 16.74 7.54999 16.68 7.57999 16.62C7.59999 16.56 7.63001 16.5 7.67001 16.44C7.70001 16.39 7.75001 16.34 7.79001 16.29C7.89001 16.2 8 16.13 8.12 16.08C8.36 15.98 8.64 15.98 8.88 16.08C9 16.13 9.10999 16.2 9.20999 16.29C9.24999 16.34 9.29999 16.39 9.32999 16.44C9.36999 16.5 9.40001 16.56 9.42001 16.62C9.45001 16.68 9.47001 16.74 9.48001 16.81C9.49001 16.87 9.5 16.94 9.5 17C9.5 17.26 9.38999 17.52 9.20999 17.71C9.10999 17.8 9 17.87 8.88 17.92C8.76 17.97 8.63 18 8.5 18Z"
                  fill="#333333"
                />
                <path
                  d="M20.5 9.83997H3.5C3.09 9.83997 2.75 9.49997 2.75 9.08997C2.75 8.67997 3.09 8.33997 3.5 8.33997H20.5C20.91 8.33997 21.25 8.67997 21.25 9.08997C21.25 9.49997 20.91 9.83997 20.5 9.83997Z"
                  fill="#333333"
                />
                <path
                  d="M15.8201 22.78C15.4401 22.78 15.0801 22.64 14.8201 22.38C14.5101 22.07 14.3701 21.62 14.4401 21.15L14.6301 19.8C14.6801 19.45 14.8901 19.03 15.1401 18.78L18.68 15.24C19.16 14.76 19.6301 14.51 20.1401 14.46C20.7701 14.4 21.38 14.66 21.96 15.24C22.57 15.85 23.39 17.09 21.96 18.52L18.4201 22.06C18.1701 22.31 17.75 22.52 17.4 22.57L16.05 22.76C15.97 22.77 15.9001 22.78 15.8201 22.78ZM20.31 15.95C20.3 15.95 20.29 15.95 20.28 15.95C20.14 15.96 19.95 16.09 19.74 16.3L16.2001 19.84C16.1701 19.87 16.12 19.97 16.12 20.01L15.9401 21.26L17.1901 21.08C17.2301 21.07 17.33 21.02 17.36 20.99L20.9 17.45C21.34 17.01 21.4 16.79 20.9 16.29C20.74 16.14 20.51 15.95 20.31 15.95Z"
                  fill="#333333"
                />
                <path
                  d="M20.9201 19.2499C20.8501 19.2499 20.7801 19.2399 20.7201 19.2199C19.4001 18.8499 18.3501 17.7999 17.9801 16.4799C17.8701 16.0799 18.1001 15.6699 18.5001 15.5499C18.9001 15.4399 19.3101 15.6699 19.4301 16.0699C19.6601 16.8899 20.3101 17.5399 21.1301 17.7699C21.5301 17.8799 21.7601 18.2999 21.6501 18.6999C21.5501 19.0299 21.2501 19.2499 20.9201 19.2499Z"
                  fill="#333333"
                />
                <path
                  d="M12 22.75H8C4.35 22.75 2.25 20.65 2.25 17V8.5C2.25 4.85 4.35 2.75 8 2.75H16C19.65 2.75 21.75 4.85 21.75 8.5V12C21.75 12.41 21.41 12.75 21 12.75C20.59 12.75 20.25 12.41 20.25 12V8.5C20.25 5.64 18.86 4.25 16 4.25H8C5.14 4.25 3.75 5.64 3.75 8.5V17C3.75 19.86 5.14 21.25 8 21.25H12C12.41 21.25 12.75 21.59 12.75 22C12.75 22.41 12.41 22.75 12 22.75Z"
                  fill="#333333"
                />
              </svg>
            </div>
          )}
        />
      </div>
    </div>
  );
};

export default CustomDatePicker;
